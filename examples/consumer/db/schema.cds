...
// Movies entity. Drives cross-service expand: local → remote ($expand=movie on
// Reviews) and cross-service expand: remote → local (Movies(...)/reviews backlink).
entity Reviews {
    key ID        : UUID;
...
// for cross-service expand: remote → local via Movies(...)/bookmarks.
entity Bookmarks {
    key ID        : UUID;
        movie     : Association to Movies;
        label     : String(100);
...
// Primary cross-service expand: local → remote showcase — list-report of
// Watchlists with `$expand=movie`.
entity Watchlists {
    key ID        : UUID;
        user      : String(100);
        movie     : Association to Movies;
...
// Wildcard + read-only. Backlinks `reviews`/`bookmarks` drive cross-service
// expand: remote → local on the Movies Object Page.
@federation.delegate
