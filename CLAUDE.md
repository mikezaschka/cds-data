...
- **Pipeline events** — `PIPELINE.READ`, `PIPELINE.MAP`, `PIPELINE.WRITE` — namespaced to avoid collision with CAP's CRUD aliases. Register hooks via `srv.before/on/after(event, pipelineName, handler)` on `data-pipeline` (`cds.connect.to('data-pipeline')`).
