...
| **Event-driven micro-runs (ADR 0009)** — batch pipeline + messaging notifications on the same name | Key or payload upserts/deletes via `executeEvent`; batch watermark preserved. CAP messaging / CloudEvents handlers call `execute` with nested `event`. | [Event-driven runs](event-driven-runs.md) |
