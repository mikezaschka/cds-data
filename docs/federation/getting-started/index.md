...
Once the basics click, these task-focused pages show how local and remote data combine in real scenarios. Each page mirrors behavior that is covered by tests in [`packages/cds-data-federation/test/integration/`](https://github.com/mikezaschka/cds-data-monorepo/tree/main/packages/cds-data-federation/test/integration) and [`packages/cds-data-pipeline/test/integration/`](https://github.com/mikezaschka/cds-data-monorepo/tree/main/packages/cds-data-pipeline/test/integration).
...
!!! tip "Prefer learning from runnable examples?"
    The [`examples/`](https://github.com/mikezaschka/cds-data-monorepo/tree/main/examples) folder contains four servers you can boot in a single command (`npm run examples:start`) with Fiori Elements UIs aggregated in a launchpad. Every tile maps to one of the getting-started scenarios.
