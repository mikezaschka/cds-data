...
| **Navigation path filters** | `$filter` through renamed or cross-service associations (e.g. `buyer/name eq 'Acme'`) is translated for same-service and [cross-service navigation](../concepts/cross-service-scenarios.md#cross-service-navigation-local-remote) (to-one and lambda). |
