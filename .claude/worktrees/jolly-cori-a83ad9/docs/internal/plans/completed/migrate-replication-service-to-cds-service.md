...
- `test/replication.test.js` — update event names in hook registrations, switch
...
- [ ] Update `test/replication.test.js` — R18, R19 hook-based tests use the
  new event names.
...
- [ ] Update `test/replication.test.js` — replace the line
...
- [ ] Audit `test/replication.test.js` R18, R19 and any other tests registering
...
2. **Does `cds.test()` in `test/replication.test.js` pick up the plugin's own
