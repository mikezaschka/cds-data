...
| [First Cache](first-cache.md) | Add response caching to a delegate with a single `cache: { ttl }` option. |

## Patterns

Once the basics click, these task-focused pages show how local and remote data combine in real scenarios. Each page mirrors behavior that is covered by tests in [`test/delegation.test.js`](https://github.com/mikezaschka/cds-data-federation/blob/main/test/delegation.test.js) and [`test/replication.test.js`](https://github.com/mikezaschka/cds-data-federation/blob/main/test/replication.test.js).
...
!!! tip "Prefer learning from runnable examples?"
    The [`examples/`](https://github.com/mikezaschka/cds-data-federation/tree/main/examples) folder contains four servers you can boot in a single command (`npm run examples:start`) with Fiori Elements UIs aggregated in a launchpad. Every tile maps to one of the getting-started scenarios.
