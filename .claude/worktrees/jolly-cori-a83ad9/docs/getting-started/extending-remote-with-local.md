...
Works for both lists and single records — unlike naive hand-written handlers, there's no "expand only allowed for one record" limitation. `$filter`, `$orderby`, and `$top` inside the expand are supported with per-parent limiting (each customer gets its own top-N bookmarks).

```http
GET /consumer/Customers?$expand=bookmarks($filter=label eq 'VIP';$orderby=label;$top=3)
```

## Flow 2: Cross-service navigation (N2)
...
This is the **Scenario C** flow described in [$expand Scenarios](../concepts/expand-scenarios.md). The logic lives in [`buildLocalAssocInfo()`, `splitLocalExpands()`, and `resolveRemoteToLocalExpands()`](https://github.com/mikezaschka/cds-data-federation/blob/main/srv/delegation/expand-remote-to-local.js). Cross-service navigation N2 is rewritten by [`rewriteRemoteToLocalNavigation()`](https://github.com/mikezaschka/cds-data-federation/blob/main/srv/delegation/cross-service-navigation.js).
...
- **Lambda filters on local children work** — `$filter=bookmarks/any(b:b.label eq 'VIP')` is resolved by [`resolveLocalLambdaFilters()`](https://github.com/mikezaschka/cds-data-federation/blob/main/srv/delegation/lambda-filters.js) which pre-queries the local DB and rewrites to a key IN on the remote query.
...
- [$expand Scenarios](../concepts/expand-scenarios.md) — the formal concept page.
