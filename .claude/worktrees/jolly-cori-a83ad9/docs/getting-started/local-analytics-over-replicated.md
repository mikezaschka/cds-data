...
Against a delegate this fails — [CAP's `cqn2odata` rejects `.groupBy` for remote services](../integration/odata.md). Against a replicated table it's a plain SQL `GROUP BY`.
...
`SELECT.distinct` [is explicitly rejected for delegated entities](../integration/odata.md) — OData has no `DISTINCT` keyword. On replicated data it's trivial.
...
- [OData Limitations](../integration/odata.md) — the exact list of CQL features unsupported on delegates.
- [Management Service](../reference/management-service.md) — schedule, run, and monitor replications.
