...
| `cache` | object | Enables response caching. See [Cache option](#cache-option). |
...
See [Integration → REST Adapter](../integration/rest.md) for worked examples of each pagination type.

## Cache option

Wraps `cds-caching` — optional peer dependency.
...
| Field | Type | Description |
|---|---|---|
| `ttl` | number (ms) | Entry lifetime. |
| `service` | string | Name of the `cds-caching` service instance. Defaults to `'caching'`. |
| `tags` | array | Static strings, dynamic data tags, value wrappers, or template tags. |

Auto-applied tag: `federation:<entityName>` — use for entity-wide invalidation via `cache.deleteByTag('federation:Customers')`.

If `cds-caching` is not installed, `cache` is silently ignored with a warning — your delegation still works.

See [Integration → cds-caching](../integration/caching.md) for tag patterns and custom-cache-service setup.
...
- [Reference → Management Service](management-service.md) — OData endpoints and programmatic API.
- [Reference → Feature Matrix](requirements.md) — detailed status for every feature.
