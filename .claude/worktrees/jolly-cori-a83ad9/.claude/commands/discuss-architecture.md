...
1. **`CLAUDE.md`** — architecture overview, plugin lifecycle, conventions, terminology, CDS quirks.
2. **`docs/architecture/*.md`** — detailed design docs:
   - `EXPAND-CONCEPT.md` — the three expand scenarios and their implementation
   - `SERVICE-QUERY-EXECUTION.md` — how CAP routes queries through services vs. DB
   - `TERMINOLOGY.md` — federation / delegation / replication definitions
3. **`docs/research/*.md`** — prior analysis that may inform the discussion:
...
4. **`REQUIREMENTS.md`** — specifically these sections:
...
5. **`docs/architecture/decisions/`** — existing ADRs, to avoid contradicting prior decisions.
6. **Source files** — if the discussion touches specific modules, read the relevant code in `srv/delegation/` or `srv/adapters/`.
...
1. **Determine the next ADR number:** List existing files in `docs/architecture/decisions/` and increment.
2. **Write the ADR** to `docs/architecture/decisions/NNNN-<slug>.md`:
...
- [REQUIREMENTS.md](../../REQUIREMENTS.md) — relevant sections
- [docs/architecture/EXPAND-CONCEPT.md](../EXPAND-CONCEPT.md) — if relevant
...
2. **Note any new requirements** that should be added to REQUIREMENTS.md (with suggested section and priority).
