...
describe('Northwind External Service Delegation', () => {
    ...
    describe('$expand Scenario A — remote-to-remote (V4)', () => {
        ...
        it('A1: should expand NwProducts -> category (to-one)', async () => {
            if (skipIfUnavailable()) return
        ...
        it('A2: should expand NwCategories -> products (to-many)', async () => {
            if (skipIfUnavailable()) return
        ...
        it('A3: should expand with $select on expanded target', async () => {
            if (skipIfUnavailable()) return
    ...
    describe('$expand Scenario B — local-to-remote (V4)', () => {
        ...
        it('B1: should expand NwProductNotes -> product (local -> Northwind)', async () => {
            if (skipIfUnavailable()) return
        ...
        it('B2: should expand with $select on Northwind remote target', async () => {
            if (skipIfUnavailable()) return
        ...
        it('B3: should batch-fetch products for notes referencing same product', async () => {
            if (skipIfUnavailable()) return
            const { data } = await GET(`${base}/NwProductNotes?$expand=product`)
    ...
    describe('$expand Scenario C — remote-to-local (V4)', () => {
        ...
        it('C1: should expand NwProducts -> notes (Northwind -> local backlink)', async () => {
            if (skipIfUnavailable()) return
        ...
        it('C2: should return empty array when product has no local notes', async () => {
            if (skipIfUnavailable()) return
        ...
        it('C3: should expand notes with $filter', async () => {
            if (skipIfUnavailable()) return
        ...
        it('C4: should expand notes with $orderby', async () => {
            if (skipIfUnavailable()) return
