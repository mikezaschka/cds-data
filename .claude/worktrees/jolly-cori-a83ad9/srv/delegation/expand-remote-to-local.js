...
// ─── Remote → Local Expand Resolution (Scenario C) ──────────────────────────────
//
// Mirror of Scenario B. When a federated entity has associations to LOCAL entities
// (e.g., Customers extended with a backlink to Bookmarks), the plugin:
//   1. Strips local expand items from the query before forwarding to remote
//   2. Fetches the main entity from the remote service (Scenario A expands pass through)
 ...
 * entities within the same service. These are Scenario C candidates.
 */
function buildLocalAssocInfo(servedEntityFullName, serviceName, federatedMap) {
    ...
    for (const [elemName, elem] of Object.entries(entityDef.elements)) {
        ...
        if (!fkInfo) {
            LOG.debug(`Cannot determine FK mapping for local assoc '${elemName}' on '${servedEntityFullName}', skipping Scenario C`)
            continue
        }
        ...
        LOG.debug(`Found Scenario C local assoc: ${servedEntityFullName}.${elemName} → ${elem.target}`)
    }
...
/**
 * Splits expand items in a query into local (Scenario C) and remote (Scenario A).
 * Returns the local expand items and a query with only remote expands.
 */
...
/**
 * Resolves Scenario C (remote→local) expands by querying the local DB
