 ...
 * mapping automatically through the projection chain (see CaLeSi docs). This
 * mapping is only needed for Scenario B (local→remote $expand resolution) where
 * the plugin must manually build remote queries and map results back, because
 * CAP cannot resolve $expand across service boundaries.
 *
 * - projectedColumns: remote field names that are projected (for $select in Scenario B)
