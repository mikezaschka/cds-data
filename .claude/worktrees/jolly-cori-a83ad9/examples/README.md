# cds-data-federation — Example Apps

Runnable, clickable demo apps for manual exploration of the plugin's features.
Complementary to the Jest-driven tests under `test/` — same patterns, but
**persistent SQLite**, **UIs attached**, and **startable with `cds watch`** so
you can walk through features interactively.

The demo models a small movies-and-streaming platform so every federation
strategy has an obvious narrative: a studio catalog is live-proxied, a hot
"trending" list is cached, a legacy V2 licensing system is reshaped with
renames, a second provider delivers streaming manifests, and REST-sourced
box-office data is replicated into the local database for analytics.
...
|---|---|---|---|
| Movies | `Movies` | `@federation.delegate` (read-only) | Live proxy of the studio V4 catalog, Scenario C backlinks to local Reviews/Bookmarks |
...
| Watchlists | `Watchlists` | Local entity with assoc to delegated `Movies` | Scenario B (local → remote `$expand=movie`, batch-fetch + stitch) |
| Reviews | `Reviews` | Local entity with assoc to delegated `Movies` | Scenario B plus Scenario C backlink from `Movies(...)/reviews` |
...
| Replication Management | *(coming soon)* | Fiori Elements UI over `DataReplicationManagementService` | Placeholder tile; UI will follow |
...
```bash
curl -X POST http://localhost:4004/federation/run \
...
`test/` runs the same plugin against real CAP providers via Jest and is the
source of truth for correctness. `examples/` is for **manual exploration**
and covers a curated subset of the features with UIs attached. When a new
user-visible feature lands, it should also land in `examples/consumer` so the
launchpad stays a working showcase (see `CLAUDE.md` → "Example apps").
