...
- Federation Monitor: http://localhost:4005/federation/Federations
...
| Entity | Pattern | Why |
|---|---|---|
| `Customers` | wildcard + local backlinks | Freshness matters. Local `notes`, `tasks`, `risk` associations enable Scenario C stitching. |
...
| `Orders` | renamed associations | Association rename (`Order_Details` → `lineItems`) demonstrates remote→remote `$expand` (Scenario A). |
...
| Entity | Pattern | Why |
|---|---|---|
| `CustomerNotes` | local + association to delegated `Customers` | Drives Scenario B (`$expand=customer` batch-fetches from Northwind). |
| `FollowUpTasks` | local + two assocs (Customers, Orders) | Cross-remote mashup in a single local query. |
| `CustomerRiskRatings` | local, backlink target | Referenced from `Customers.risk` — drives Scenario C. |
...
|---|---|---|---|---|
| My Work | Customer Notes | List Report | `CustomerNotes` | Local entity with Scenario B expand to Northwind |
| Master Data | Customers | List Report | `Customers` | Live V4 delegation |
| Intelligence | Customer 360 | Object Page | `Customers` | Object Page facets for notes / tasks / risk (Scenario C) |
| Intelligence | Sales Analytics | ALP | `SalesOrders` | Chart + KPIs over replicated data (`$apply/groupby` works) |
| Admin | Federation Monitor | List Report | `Federations` | Plugin management service with run history facet |
...
and risk, populated by the plugin's Scenario C resolver.
...
```bash
curl -X POST http://localhost:4005/federation/run \
