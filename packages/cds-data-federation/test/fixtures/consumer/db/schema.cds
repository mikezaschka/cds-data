...
// Service Integration docs: "OData doesn't support denormalization."
...
// ─── CQN adapter (local source table for CQN tests) ────────────────────────
//
// A local SQLite-backed table used as the *source* for CqnAdapter tests. It
// stands in for a secondary reporting DB / CAP-wrapped legacy database: from
// the engine's perspective `cds.connect.to('db')` is a CQN-native service.
...
// Target for the `replicate` use case over CqnAdapter. Row-preserving copy.
...
// Target for the `materialize` use case: per-customer aggregate snapshot
// rebuilt from `SourceOrders` on each run.
...
// Entity-level cache (SQLite + pipeline full read). Long TTL keeps tests deterministic.
@federation.delegate: { cache: { strategy: 'entity', ttl: 600000, batchSize: 500 } }
