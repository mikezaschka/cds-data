 ...
 *   2. Cross-service expand resolution (local → remote: batch-fetch + stitch)
 *   3. Optional response caching (per-query cds-caching or entity-level SQLite pipeline)
 *
 * @param {Array} federationConfigs - configs from annotation scanner
 * @param {Object} viewMappingRegistry - mapping registry (only needed for cross-service expand: local → remote)
 */
async function registerFederationHandlers(federationConfigs, viewMappingRegistry = {}) {
    ...
    // Handler registration is deferred so that cross-service expand: remote → local
    ...
    for (const config of federationConfigs) {
        ...
        pendingHandlers.push({
            ...
            servedFullName,
            writeFlags,
            entityCacheMeta: config.entityCache,
        })
    }

    // Phase 2: Register delegate handlers (with cross-service expand: remote → local assoc info)
    ...
    // Phase 3: Register cross-service expand: local → remote resolvers
