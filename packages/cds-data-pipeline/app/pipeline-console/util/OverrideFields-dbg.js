sap.ui.define([], function () {
  ...
  function overrideFieldKind(path) {
    switch (path) {
      case "enabled":
        return "boolean";
      case "mode":
  ...
  function parseScheduleForEdit(current) {
    const defaults = {
  ...
  function createOverrideEditState(path, current) {
    ...
    switch (kind) {
      ...
      case "schedule":
        ...
        break;
      case "json":
        state.textValue = current == null ? "{}" : typeof current === "object" ? JSON.stringify(current, null, 2) : String(current);
        break;
      default:
        break;
    }
  ...
  function overrideFieldI18nKey(path, prefix) {
    return `${prefix}_${path.replace(/\./g, "_")}`;
  }
  function buildOverrideValue(state) {
    switch (state.kind) {
      case "boolean":
        return {
      ...
      case "deltaMode":
        if (!state.selectKey) {
          return {
            errorKey: "overrideSelectRequired"
          };
        }
        return {
      ...
      case "integer":
        {
          ...
          if (Number.isNaN(parsed)) {
            return {
              errorKey: "overrideIntegerInvalid"
            };
          }
          return {
            value: parsed
      ...
      case "schedule":
        if (state.scheduleKind === "cron") {
          ...
          if (!cron || cron.split(/\s+/).length !== 5) {
            return {
              errorKey: "scheduleCronInvalid"
            };
          }
          return {
            value: {
              cron,
              engine: state.scheduleEngine || "queued"
            }
          };
        }
        {
          ...
          if (Number.isNaN(every) || every <= 0) {
            return {
              errorKey: "scheduleInvalid"
            };
          }
          return {
            value: {
              every,
              engine: state.scheduleEngine || "spawn"
            }
          };
        }
      case "json":
        try {
          return {
            value: JSON.parse(state.textValue)
          };
        } catch {
          return {
            errorKey: "overrideJsonInvalid"
          };
        }
      default:
        return {
  ...
  __exports.overrideFieldKind = overrideFieldKind;
  __exports.createOverrideEditState = createOverrideEditState;
  __exports.overrideFieldI18nKey = overrideFieldI18nKey;
  __exports.buildOverrideValue = buildOverrideValue;
  return __exports;
});
//# sourceMappingURL=OverrideFields-dbg.js.map
