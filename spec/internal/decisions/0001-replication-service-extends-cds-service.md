...
**Date:** 2026-04-17
**Status:** Accepted
**Amended:** 2026-04-19 by [ADR 0005](./0005-reposition-engine-as-cds-data-pipeline.md)

> **Amendment note ([ADR 0005](./0005-reposition-engine-as-cds-data-pipeline.md), 2026-04-19).** The structural decision recorded here — extending `cds.Service`, single-winner `on` handler routing, `srv.dispatch(req)` for non-CRUD events, `req.reply(value)` interceptor semantics, `before → on → after` over the three phases — is **unchanged**. Only names change:
>
> - Service class `DataReplicationService` → **`DataPipelineService`**.
> - Event namespace `REPLICATE.READ / MAP / WRITE` → **`PIPELINE.READ / MAP / WRITE`**.
> - Service stub `cds.connect.to('DataReplicationService')` → **`cds.connect.to('DataPipelineService')`**.
>
> Read this ADR as describing `DataPipelineService` / `PIPELINE.*` everywhere it currently says `DataReplicationService` / `REPLICATE.*`. The engine package itself is renamed to `cds-data-pipeline` ([ADR 0005](./0005-reposition-engine-as-cds-data-pipeline.md)).
