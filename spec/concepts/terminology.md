...
Per [ADR 0005](../internal/decisions/0005-reposition-engine-as-cds-data-pipeline.md), the engine package talks about **pipelines**, not replications; replication is one specific *shape* of pipeline. Per [ADR 0007](../internal/decisions/0007-infer-pipeline-intent-from-config-shape.md), the pipeline's `kind` is **derived** by the engine from the configuration shape — consumers do not pass `kind` to `addPipeline`. The four terms are deliberately narrow:
...
See [`concepts/inference.md`](../../packages/cds-data-pipeline/spec/concepts/inference.md) on the engine site for the full derivation table and registration-time validation matrix.
