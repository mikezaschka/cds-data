...
The cache wraps the outbound `remote.run(req.query)` call — including the entire `$expand` payload — via the peer dependency [`cds-caching`](https://github.com/mikezaschka/cds-caching). No expand-specific logic.
