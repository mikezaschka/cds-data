...
| 4.8 Scheduling & Triggers | 6 | 3 | 1 | 1 | 1 |
| 4.9 CQL Safety | 1 | 1 | 0 | 0 | 0 |
| 4.10 Resilience | 11 | 3 | 1 | 7 | 0 |
| 4.11 Observability | 8 | 4 | 1 | 2 | 1 |
| 4.12 Security | 3 | 1 | 0 | 1 | 1 |
| 4.13 Management API | 9 | 8 | 0 | 0 | 1 |
...
| 4.1.6 | **Static where clause** -- support `where condition` in consumption view projections to apply a permanent filter to all queries. The annotation scanner extracts `projection.where` from the CSN and stores it as `staticWhere` in the view mapping. The delegate handler detects `staticWhere` and routes through `runDirectRemoteQuery()`, which injects the static filter (already in remote field names) into the remote CQN after local→remote field translation. This correctly handles wildcard projections (`ActiveCustomers where blocked = false`), restricted projections with non-projected filter fields (`ElectronicsProducts where category = 'Electronics'`), and delegated expands whose target views restrict or rename columns (`ShippedOrders` / `ShippedOrdersV2`). Because this path rebuilds the query rather than resolving CAP's projection chain, it also re-applies what the chain would have contributed: the key predicate carried on the from-segment (`Entity('K')`), the static filter of an expand target view (`ShippedOrdersScoped` expanding into `ElectronicsProducts`), and parenthesization of client clauses so a top-level `or` cannot widen the permanent scope. OData V2 cannot carry nested expand options, so the direct path selects fields needed for local evaluation, applies translated predicates, ordering, `$skip`, and `$top` per expanded collection, and removes evaluation-only fields from the response. Decimal and Int64 comparisons use the target CSN type and preserve the precision of IEEE-754-compatible strings; ordered comparisons with `null` follow OData filter semantics and do not match. Unsupported CQN predicate forms fail the request rather than widening the permanent scope. Tested with: basic filtering, $count, $orderby, combined client $filter + static where, read by (renamed) key, client `or` filters, renames + static where, V4/V2 `$expand`, recursive renamed nested expands on the direct path, V2 `in` scopes, client expand filters, explicit selects, ordering, and limits, expand target static where with and without a client expand filter, renamed V4 fields in nested expand `$filter` / `$orderby`, and renamed association foreign keys in `$select` / `$filter` / `$orderby`. **Limitation:** the direct path is not yet compatible with HCQL remotes — such queries are rejected with "Cannot determine target entity of query" (skipped test in `test/integration/hcql/hcql-flatten.test.js`). **Note:** CAP's [old "Consuming Services" docs](https://cap.cloud.sap/docs/guides/services/consuming-services#supported-projection-features) explicitly list `where` conditions in projections as "not supported" for remote services. This is a plugin value-add beyond CAP's native capabilities. | P1 | Implemented |
...
| 4.2.6 | **$filter / $orderby / $select / $expand passthrough** -- query parameters are forwarded and translated by CAP's projection chain. Includes delegated expand. When static `where` requires the direct-query path, the plugin emits scalar `$select` columns plus explicitly narrowed expand columns so OData V2 does not receive `nav/*` and target consumption-view restrictions remain effective. Projected associations contribute their remote foreign keys instead of a bare navigation ref, and renamed foreign keys (`buyer_ID` for `customer as buyer`) are translated in both directions so the direct path stays field-for-field compatible with CAP's projection chain. | P0 | Implemented |
| 4.2.7 | **Navigation path filter translation** -- when `$filter` uses a navigation path through a renamed association (e.g., `buyer/name` where `buyer` is renamed from `customer`), CAP does not translate the association name. The plugin's `translateNavigationFilters()` pre-translates same-service navigation paths before forwarding. Cross-service navigation (local→remote, Scenario B) uses `resolveRemoteNavigationFilters()` to query the remote service for matching keys and rewrite the navigation filter to a local FK IN filter. Cross-service navigation (remote→local, Scenario C) uses `resolveLocalNavigationFilters()` to query the local DB for matching FKs and rewrite to a source-key IN filter before the remote call. Lambda operators (`any`/`all`) on cross-service to-many associations use `resolveRemoteLambdaFilters()` / `resolveLocalLambdaFilters()`. | P1 | Implemented (same-service + Scenario B to-one + Scenario B lambda + Scenario C to-one + Scenario C lambda) |
...
| **Navigation path $filter** (e.g., `buyer/name eq 'X'`) | Yes (SQL JOINs) | Yes (V4 only, plugin translates assoc renames) | Yes (V4, plugin query splitting via resolveRemoteNavigationFilters) | Yes (V4, plugin query splitting via resolveLocalNavigationFilters) |
...
| 4.8.3 | **Event-driven single-record sync** -- subscribe to SAP Business Events / CloudEvents (e.g., `sap.s4.beh.businesspartner.v1.BusinessPartner.Changed.v1`) to trigger per-record replication. Use `cds.outboxed()` transactional outbox pattern for reliable event delivery with automatic retry ([Service Integration guide: Transactional Outbox](https://cap.cloud.sap/docs/guides/integration/calesi)). **Partial ([ADR 0013](../internal/decisions/0013-event-driven-pipeline-runs.md), API-first):** `execute`/`executeEvent` run per-record micro-runs (`event.read: 'key'\|'payload'`, `event.action: 'upsert'\|'delete'`) without advancing the batch watermark; a `messaging.on(...) → executeEvent` bridge is shown in `examples/07-event-driven-runs`, with `event-execute.test.js` coverage. Still open: declarative topic subscription wiring (`source.event.topic`) and `cds.outboxed()`-based delivery/retry. | P1 | In progress (ADR 0013 executeEvent micro-runs + messaging-bridge example + tests; declarative subscription + cds.outboxed delivery deferred) |
...
| 4.8.5 | **On-read trigger** -- lazy replication on first access (with TTL-based staleness check) | P2 | Removed (superseded by the entity-cache strategy 4.3.8, which already does TTL-based on-read refill for delegate; a separate lazy-replicate trigger adds no distinct value) |
...
| 4.11.6 | **Request-level tracking** -- optional per-batch tracking with source/target data snapshots persisted per run | P1 | Removed (superseded by the on-demand data inspector 4.13.6/4.13.7, which provides live source/target previews via `inspectData`; persisted per-batch snapshots add storage cost without a driving use case) |
...
| 4.12.2 | **Sensitive data filtering** -- annotation to exclude fields from replication (e.g., `@federation.exclude`) | P1 | Removed (superseded by consumption-view column restriction — fields not projected are never fetched or replicated, so a dedicated exclude annotation is redundant) |
...
| 4.13.5 | **Fiori Elements UI** -- optional monitoring dashboard (annotations-driven) | P2 | Removed (superseded by the Pipeline Console 4.13.6, which is the single monitoring UI for all cases; no separate Fiori Elements dashboard) |
...
| 4.16.4 | **Launchpad** -- `examples/consumer/app/launchpage.html` aggregates all FE apps in a `sap.ushell` sandbox following the risk-management sample pattern; pipeline monitoring is provided by the embedded Pipeline Console (4.13.6) rather than a separate Fiori Elements tile | P1 | Implemented |
| 4.16.5 | **Management UI** -- Fiori Elements UI over `DataPipelineManagementService` (Pipelines, PipelineRuns) with actions to trigger runs and flush state | P2 | Removed (superseded by the Pipeline Console 4.13.6; examples embed the Pipeline Console rather than a separate Fiori Elements management app) |
...
Symmetric to §4.6 (source adapters, READ phase). The WRITE phase **no longer** hard-codes `cds.connect.to('db')` — [`Pipeline._defaultWriteHandler`](../../packages/cds-data-pipeline/srv/lib/Pipeline.js) now dispatches through a resolved `targetAdapter` (`BaseTargetAdapter` subclass) selected by [`srv/adapters/targets/factory.js`](../../packages/cds-data-pipeline/srv/adapters/targets/factory.js). The `TargetAdapter` capability layer generalises writes to arbitrary `target.service` kinds (other CAP services, OData remotes, REST backends, CQN-native services) and unlocks the three registration-time validation rows deferred by [ADR 0007 §"Registration-time validation matrix"](../internal/decisions/0007-infer-pipeline-intent-from-config-shape.md) (rows 6–8), which are now enforced (see 4.17.8). The `DbTargetAdapter` and `ODataTargetAdapter` + the factory + capability-validated registration are implemented; a dedicated `RestTargetAdapter` and `CqnTargetAdapter` remain open.

**Contract note:** the shipped `BaseTargetAdapter` uses the method names `writeBatch(records, { mode, target })`, `truncate(target)`, and `deleteSlice(target, predicate)`, and advertises capability flags `keyAddressableUpsert` / `batchInsert` / `batchDelete` / `truncate` via a `capabilities()` method. This differs from the original ADR 0008 wording (`deleteBatch`, `supports*` names) below; the code is authoritative.
...
| # | Feature | Priority | Status |
|---|---|---|---|
| 4.17.1 | **`BaseTargetAdapter` contract** -- documented interface for user-defined and built-in target adapters. Shipped contract: `writeBatch(records, { mode, target })` (idempotent per-batch), `truncate(target)` (full-refresh), `deleteSlice(target, predicate)` (partial-refresh), and `capabilities()`. Symmetric to `BaseAdapter.readStream(tracker)` in §4.6.4. Class at [`srv/adapters/targets/BaseTargetAdapter.js`](../../packages/cds-data-pipeline/srv/adapters/targets/BaseTargetAdapter.js). (Method/flag names differ from the ADR 0008 draft — see the §4.17 contract note.) | P1 | Implemented |
| 4.17.2 | **Capability flag declaration** -- each adapter's `capabilities()` declares `keyAddressableUpsert` (enables `mode: 'delta'`), `batchDelete` / `truncate` (enables `mode: 'full'`), `batchInsert` (enables `source.query` snapshot writes). Consumed by `DataPipelineService._validateTargetCapabilities` (invoked at registration from `Pipeline.init`) to produce config-grounded registration errors per [ADR 0007 rows 6–8](../internal/decisions/0007-infer-pipeline-intent-from-config-shape.md). Tests: `validation.test.js` rows 6–8. | P1 | Implemented |
| 4.17.3 | **DB target adapter** -- default for `target.service: 'db'` (or unset). Implements `writeBatch` via `INSERT.into` (`mode: 'snapshot'` for `source.query` shape) or `UPSERT.into` (`mode: 'upsert'` for `source.entity` shape). Also serves secondary SQL CQN targets (`sqlite`/`postgres`/`hana`) resolved via `target.service`. Advertises `keyAddressableUpsert`, `batchInsert`, `batchDelete`, `truncate`. Preserves Req 4.4.4 semantics. Lifted from `_defaultWriteHandler` into [`DbTargetAdapter`](../../packages/cds-data-pipeline/srv/adapters/targets/DbTargetAdapter.js). | P0 | Implemented |
| 4.17.4 | **OData target adapter** -- `target.service` kinds `odata` / `odata-v2`, auto-selected by the factory (`target.kind` or `service.options.kind`). Class exists and is integration-tested (move-to-service). **Gap vs spec:** the current `writeBatch` uses per-row `INSERT.into` (append-only via CAP's remote runtime) rather than PUT-by-key with 404 → POST fallback, so per-record UPSERT idempotency is not yet met, and it advertises `batchInsert: true`. No explicit OData `$batch` construction. Class at [`ODataTargetAdapter.js`](../../packages/cds-data-pipeline/srv/adapters/targets/ODataTargetAdapter.js). | P1 | In progress (adapter + factory + test; PUT/404→POST idempotency not yet implemented) |
| 4.17.5 | **REST target adapter** -- `target.service` kind `rest`. User-supplied `writeBatch(records)` closure on the `target` config (symmetric to `source.query` in §4.6.3). No implicit idempotency; the user owns the write semantics. `deleteBatch` similarly user-supplied. Capabilities: declared by the user on the config. Explicit `source` / `target` in annotation (no CAP model needed). **Movement only** — like the source-side REST adapter, CAP does not translate CQN to REST. No built-in `RestTargetAdapter`; the custom `target.adapter` class escape hatch covers this case today. | P1 | Not started |
| 4.17.6 | **CQN target adapter** -- for CQN-native `cds.requires` services (secondary DB, CAP-wrapped legacy DB, sibling CAP service). Uses `UPSERT.into` / `DELETE.from` against `target.service.run(cqn)`. Enables service-to-service replication between two CAP services with identical semantics to a local-DB target. See [ADR 0004](../internal/decisions/0004-scope-cqn-adapter-to-cds-data-replication.md) for the source-side twin. **Partial:** secondary SQL databases (`sqlite`/`postgres`/`hana`) already route through `DbTargetAdapter` with CQN semantics; a dedicated `CqnTargetAdapter` and auto-dispatch for CAP sibling (non-SQL, non-OData) CQN services are not yet built (they require an explicit `target.adapter` today). | P1 | In progress (secondary-SQL CQN targets via DbTargetAdapter; dedicated CqnTargetAdapter + sibling-service dispatch pending) |
| 4.17.7 | **Target adapter factory** -- auto-selects adapter by `target.service` / `target.kind` (analogous to §4.6.5). Wired into `Pipeline.init` → `_defaultWriteHandler`, which dispatches to the resolved `targetAdapter.writeBatch(...)` instead of a hard-coded `cds.connect.to('db')`. Custom targets register via `target.adapter` class ref; unknown non-`db`/non-OData kinds throw at registration time (no silent DB fallback). [`factory.js`](../../packages/cds-data-pipeline/srv/adapters/targets/factory.js) + `target-adapter-factory.test.js`. | P1 | Implemented |
| 4.17.8 | **Capability-validated registration** -- engine compares `mode` / `source.query` against the resolved target adapter's capability flags and rejects incoherent combinations at registration (`_validateTargetCapabilities`, called from `Pipeline.init`). Implements [ADR 0007 validation matrix rows 6–8](../internal/decisions/0007-infer-pipeline-intent-from-config-shape.md): `mode: 'delta'` against a target without `keyAddressableUpsert`; `mode: 'full'` against a target without `truncate`/`batchDelete`; `source.query` against a target without `batchInsert`. Tested (`validation.test.js` rows 6–8). Minor gaps: the `delta` object and partial-refresh (`batchDelete`) gates are not separately validated. | P1 | Implemented |
